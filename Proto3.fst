...
let pty_wrap (#t1:Type) (#t2:Type) (f:t1 -> t2) (d:t2) (a:proto_dec t1) = 
  match a with 
  | IMPLICIT (Some a') -> IMPLICIT (Some (f a'))
  | IMPLICIT None -> IMPLICIT (Some d)
  | OPTIONAL (Some a') -> OPTIONAL (Some (f a'))
  | OPTIONAL None -> OPTIONAL None 
  | REPEATED l -> REPEATED (List.map f l)

let proto_ty_int_sint32 = pty_wrap (int_sint 32) (zwz 32)
let proto_ty_int_sint_test_i = proto_ty_int_sint32 (IMPLICIT (Some (-9)))
let proto_ty_int_sint_test_i' = proto_ty_int_sint32 (IMPLICIT (None))
...
type val_rel : proto_ty -> proto_ty -> Type = 
  ...
  | ValStrByt : 
    v1:(proto_dec string) -> 
    v2:(proto_dec bytes){v2 = pty_wrap String.list_of_string empty_bytes v1} -> 
  ...
  | ValUintChgW : 
    ...
    #w2:width ->
    v1:(proto_dec (uw w1)) ->
    v2:(proto_dec (uw w2)){v2 = pty_wrap (uint_change_w w2) (uwz w2) v1} ->
    val_rel (UINT w1 v1) (UINT w2 v2)
  | ValIntChgW : 
    #w1:width ->
    #w2:width ->
    v1:(proto_dec (zw w1)) ->
    v2:(proto_dec (zw w2)){v2 = pty_wrap (int_change_w w2) (zwz w2) v1} ->
  ...
  | ValUintInt : 
    #w:width ->
    v1:(proto_dec (uw w)) ->
    v2:(proto_dec (zw w)){v2 = pty_wrap (uint_int w) (zwz w) v1} -> 
    val_rel (UINT w v1) (INT w v2)
  | ValIntUint : 
    #w:width ->
    v1:(proto_dec (zw w)) ->
    v2:(proto_dec (uw w)){v2 = pty_wrap (int_uint w) (uwz w) v1} -> 
    val_rel (INT w v1) (UINT w v2)
  | ValUintSint : 
    #w:width ->
    v1:(proto_dec (uw w)) ->
    v2:(proto_dec (zw w)){v2 = pty_wrap (uint_sint w) (zwz w) v1} -> 
    val_rel (UINT w v1) (INT w v2)
  | ValIntSint : 
    #w:width ->
    v1:(proto_dec (zw w)) ->
    v2:(proto_dec (zw w)){v2 = pty_wrap (int_sint w) (zwz w) v1} -> 
  ...
  | ValUintBool :
    #w:width ->
    v1:(proto_dec (uw w)) ->
    v2:(proto_dec bool){v2 = pty_wrap (fun (u:uw w) -> u <> 0) false v1} ->
    val_rel (UINT w v1) (BOOL v2)
  | ValBoolUint :
    #w:width ->
    v1:(proto_dec bool) ->
    v2:(proto_dec (uw w)){v2 = pty_wrap (idnuw w) (uwz w) v1} ->
    val_rel (BOOL v1) (UINT w v2)
  | ValIntBool : 
    #w:width ->
    v1:(proto_dec (zw w)) ->
    v2:(proto_dec bool){v2 = pty_wrap (fun (u:zw w) -> u <> 0) false v1} ->
    val_rel (INT w v1) (BOOL v2)
  | ValBoolInt :
    #w:width ->
    v1:(proto_dec bool) ->
    v2:(proto_dec (zw w)){v2 = pty_wrap (idnzw w) (zwz w) v1} ->
  ...
  | ValAddOpt :
    ...
    val_rel (pty (IMPLICIT (Some v))) (pty (OPTIONAL (Some v)))
  | ValRmOpt :
    #t:eqtype ->
    #pty:(proto_dec t -> proto_ty) ->
    #v:t ->
    pty1:(proto_dec t){is_OPTIONAL_v pty1 v} ->
    val_rel (pty (OPTIONAL (Some v))) (pty (IMPLICIT (Some v)))
  | ValAddRep :
    #t:eqtype ->
    #pty:(proto_dec t -> proto_ty) ->
    #v:t ->
