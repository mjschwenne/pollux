...
Equations fold_right {A B} (f : A -> B -> B) (b : B) (l : list A) : B :=
  fold_right f b [] := b;
  fold_right f b (a :: l) := fold_right f (f a b) l.
...
Proof.
  funelim (length l); autorewrite with length app.
  - reflexivity.
...
Proof.
  funelim (nth_option n l); autorewrite with length nth_option.
  + reflexivity.
  + intros. inversion H.
  + reflexivity.
  + intros. apply le_S_n in H0. apply H in H0. assumption.
Qed.
...
Proof.
  ...
  + autorewrite with length in H.
    pose proof (PeanoNat.Nat.lt_irrefl 0) as Hcontra.
    contradiction.
  + autorewrite with length in H.
    apply PeanoNat.Nat.lt_succ_l in H.
    pose proof (PeanoNat.Nat.nlt_0_r n) as Hcontra.
    contradiction.
  + apply H. apply PeanoNat.lt_S_n.
    autorewrite with length in H0.
    assumption.
Qed.
...
Proof.
  funelim (nth_option n l); simp nth_option app length.
  + reflexivity.
  + intro H.
    pose proof (PeanoNat.Nat.nle_succ_0 (length l)) as Hcontra.
    contradiction.
  + reflexivity.
  + intro Hlen. apply le_S_n in Hlen. apply (H l') in Hlen.
    rewrite Hlen. reflexivity.
Qed.

Lemma distr_rev {A} (l l' : list A) : rev (l ++ l') = rev l' ++ rev l.
Proof.
  funelim (rev l); simp rev app.
...
Proof.
  funelim (head_option l); simp head.
  contradiction.
Qed.

(* I'm a bit surprised that my first attempt here was that concise... *)
...
Proof.
  funelim (filter l p); simp filter length.
  * apply le_n. 
  * apply le_n_S. assumption.
  * apply PeanoNat.Nat.le_le_succ_r. assumption.
Qed.

Equations find_dictionary {A B} (eq : A -> A -> bool) (key : A)
          (l : list (A * B)) : option B :=
find_dictionary eq key [] := None;
find_dictionary eq key ((k, v) :: l) with eq key k => {
  | true := Some v
  | false := find_dictionary eq key l
}.
