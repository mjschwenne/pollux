...
`Pollux.Proto` is the real-protobuf successor to `InterParse`. Two pieces exist: the **sealed descriptor kernel** (`Proto/Descriptor.lean`) and the **value layer** (`SortedMap.lean`, `Value.lean`, `Validity.lean`, `Transform.lean`). The design rationale, including the experiments that ruled out the alternatives, is in `lean/proto-design.org` (source material for the report). Operative rules:
